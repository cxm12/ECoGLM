...
from data import SEEDIVDataset_TT#0 as SEEDIVDataset_TT
...
def testtoken(epoch, loader=None):
    if loader is None:
        ...
        full_dataset.set_mode('test')
        loader = DataLoader(full_dataset, batch_size=32, shuffle=False)  # 可以用更大 batch
        # token_model.load_state_dict(torch.load(tokenmodel_path.replace(".pth", "_best.pth"), map_location=device))
        # 71.02%
        # token_model.load_state_dict(torch.load(tokenmodel_path, map_location=device))
        # 67.80%
        # print("Load best model from", tokenmodel_path.replace(".pth", "_best.pth"))    
...
def trainVQVAE():
    ...
    for epoch in range(start_epoch, EPOCHS):
        ...
        current_inter_sim = get_inter_class_similarity(model, val_data, device, num_samples=len(val_data))
        ...
        # 7. 定期诊断
        if (epoch) % 20 == 0:
            diagnose_vq_collapse_multiClass(val_data, model, device, num_samples=20, num_classes=num_classes)  # ← 传入 4
...
def testVQVAE(save_path0, isbest=True, channel_idx=0):
    ...
    diagnose_vq_collapse_multiClass(test_data, model, device, num_samples=20,  # 每类采 20 个
    ...
    analyze_token_distribution(model, test_loader, test_loader, codebook, device)
...
if __name__ == "__main__":
    ...
    save_path = "./checkpoints/Stanford/VQTokenizer/SEED-IV_c%d_4_1/" % (codebook) 
    ...
    print("... Build Model ...")
    aligner = TokenAlignmentTrainer()    
    model.load_state_dict(torch.load(VQVAEmodelpath)['model_state_dict'])
    vqvae = model
    
    # save_path = "./checkpoints/Stanford/VQTokenizer/SEED-IV_c%d_4_FT/" % (codebook) 
    # os.makedirs(save_path, exist_ok=True)
    ...
    token_model = TransformerVQToTokenModel6_Robust64(  # _Robust(  # 
        ...
        num_classes=num_classes, # vocab_size=len(aligner.tokenizer), 
        max_seq_len=actual_seq_len, num_channels=num_channels).to(device)
        
    train_token()
    # train_token_model_joint()
    print("\n--- 执行完整验证 Pipeline ---")
    testtoken(epoch=999)
